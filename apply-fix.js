...
// 1. Replace getCardsDocument
const startIdx = content.indexOf('  const getCardsDocument = (includePrintScript: boolean) => {');
const endIdx = content.indexOf('  const handleBulkPrint = () => {');
...
`;
    content = content.substring(0, startIdx) + newGetCardsContent + content.substring(endIdx);
    fs.writeFileSync(file, content, 'utf8');
    console.log('Replaced getCardsDocument');
} else {
    console.log('Could not find start or end index for getCardsDocument');
}

// 2. Replace handleDownloadHtmlPdf
let updatedContent = fs.readFileSync(file, 'utf8');
const dlStart = updatedContent.indexOf('  const handleDownloadHtmlPdf = async () => {');
const dlEnd = updatedContent.indexOf('  const handleExportCsv = () => {');
if (dlStart !== -1 && dlEnd !== -1) {
    updatedContent = updatedContent.substring(0, dlStart) + `  const handleDownloadHtmlPdf = () => {
    handleBulkPrint();
  };

` + updatedContent.substring(dlEnd);
    fs.writeFileSync(file, updatedContent, 'utf8');
    console.log('Replaced handleDownloadHtmlPdf');
} else {
    console.log('Could not find start or end index for handleDownloadHtmlPdf');
}
